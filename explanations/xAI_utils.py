...
# --------------------------------------------------------
# Port Explainer Baselines 

class Explainer(ABC):
    @abstractmethod
    def explain_node_task(self, task, graph):
        """Returns edge importance scores for the node at index in graph."""

    @abstractmethod
    def explain_graph_task(self, task, graphs):
        """Returns edge importance scores for the graph at graph_index in graphs."""


class ProxyExplainerImpl(Explainer):
    def __init__(self, epochs, reg_coefs):
        self.epochs = epochs
    ...
    def explain_graph_task(self, task: InductiveGraphClassification, graphs):
        ...
        self.explainer = PROXYExplainer(
            ...
            reg_coefs=self.reg_coefs, epochs=self.epochs
        )
        graph_masks = []
