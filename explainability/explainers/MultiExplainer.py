...
class MultiExplainer(PGExplainer):
    ...
    def __init__(
            self, epochs, hidden_size, lr, mean_regularization, 
            entropy_regularization, tau, reparameterization_samples, 
            loss_f=torch.nn.BCELoss, threshholder_function=otsu_threshold):

        super().__init__(epochs, hidden_size, lr, mean_regularization, 
                         entropy_regularization, tau, 
    ...
    def explain_graph_task(self, model, graphs):
        while True:
            ...
            hard_masks = [
                m > self.threshholder_function(m) 
                for m in masks
            ]
